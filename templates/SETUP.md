...
- [ ] Let agents work Triage: an agent with the Linear connector (a Claude project, Grok Bot, the Cursor Project coordinator) expands one-liners into draft acceptance criteria, dedupes, labels, and proposes priority. **Triage → Todo is the human approval click** — that's the only tracker action agents don't take. Keep Triage enabled on every team for this reason.
