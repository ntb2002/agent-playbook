...
- [ ] Let agents work Triage: an agent with the Linear connector (a Claude project, Grok Bot, the Cursor Project coordinator) expands one-liners into draft acceptance criteria, dedupes, labels, and proposes priority. Agents never change status by hand — accepting out of Triage and promoting into Todo are human clicks; In Progress / Done come from the PR. Keep Triage enabled on every team for this reason.
