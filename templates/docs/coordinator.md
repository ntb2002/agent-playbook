...
2. **Expand** Deep-lane issues with `/plan-feature <issue-id>` into `plans/features/<issue-id>-<slug>.md`. Standard-lane issues need no plan — the issue is the spec. If acceptance criteria are missing, draft them on the issue (marked draft) and wait for the human to approve — do not build on invented product intent.
