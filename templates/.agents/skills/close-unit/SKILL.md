...
5. Open the PR (never commit to `main`): ensure work is on a branch `<phase>/<unit>`; `git add`, commit (concise why-focused message), `git push -u origin HEAD`; `gh pr create` with the body filled from `.github/pull_request_template.md` (paste the gate checklist + evidence). **Stop at the open PR — do not merge.** Return the PR URL.
