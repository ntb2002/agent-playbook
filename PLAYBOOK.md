...
- **Issues are Linear issues.** GitHub Issues are unused. `/start-unit SP-7` means fetch `SP-7` from Linear, not search GitHub, Notion, or `plans/` for a ticket. `plans/features/` is the execution spec written *after* `/plan-feature` for an already-accepted Linear issue — it is not where agents look to discover work.
...
- **Proportional rigor — three lanes.** Ceremony scales with risk; the proof never does. Every lane keeps the non-negotiables: a Linear issue as the record, a branch, a PR titled with the issue id, and gate evidence (test + artifact) on the PR. What varies is how much approval and planning sits in front of the build:

  | Lane | When | Path |
  |---|---|---|
  | **Fast** | Cause already understood, small diff, no schema / auth / user-facing prompts / `DECISIONS.md` call — typically a bug found and diagnosed while pairing with a code agent | Human says "go" in chat → agent files the issue (into In Progress, with repro + cause + gate) → branch → fix + test + evidence → PR. No Triage round-trip, no plan file. |
...
- **Who shapes an issue depends on what they can see.** A tracker agent without code access (e.g. Linear Agent) shapes the *product* side — problem, impact, acceptance criteria, `## Needs human` — and never guesses root causes or implementation. A code-aware agent shapes the *technical* side: investigates, posts findings on the issue, and writes the plan when the Deep lane needs one. Bugs found in a coding session are filed by that coding agent, which already has the repro and the cause.
